...
// Include helper functions from envios.php
require_once 'envios.php';

// Tool paths for PDF processing
$pdftotext = '"C:\\poppler\\poppler-25.12.0\\Library\\bin\\pdftotext.exe"';

// Function to extract text from PDF
function extrairTextoPdfNova($caminhoPdf) {
    global $pdftotext;
    ...
    $cmd = $pdftotext . ' -layout ' . escapeshellarg($caminhoPdf) . ' ' . escapeshellarg($tmp) . ' 2>&1';
    ...
    if ($texto === false) {
        throw new Exception('Erro ao ler o arquivo de texto extraído.');
    }
    
    return normalizarTextoPdf($texto);
}

// Function to extract guide transport data from PDF text
function extrairDadosGuiaTransporteNova($textoExtraido, $pdo, $parceirosInventario) {
    $dados = [
        'documento' => '',
        'num_documento' => '',
        'data_documento' => '',
    ...
    $cabecalho = extrairCabecalhoGuia($textoExtraido);
    $dados['documento'] = $cabecalho['documento'] ?? '';
    $dados['num_documento'] = $cabecalho['numero_documento'] ?? '';
    $dados['data_documento'] = $cabecalho['data_documento'] ?? '';
    
    // Try to find partner from the extracted data
    if (!empty($cabecalho['destinatario_nome'])) {
        $nomeDestino = strtolower($cabecalho['destinatario_nome']);
        foreach ($parceirosInventario as $parceiro) {
            if (stripos($nomeDestino, strtolower($parceiro)) !== false) {
    ...
    // Extract items
    $itens = extrairItensTextoPdf($textoExtraido);
    
    // Get product catalog (from global scope in app.php)
    $catalogoProdutos = $GLOBALS['catalogoProdutos'] ?? [];
    ...
    foreach ($itens as $item) {
        ...
        foreach ($catalogoProdutos as $cat => $produtos) {
            if (in_array($produto, $produtos, true)) {
                $categoria = $cat;
                break;
            }
        }
        
        if ($categoria === '' && !empty($produto)) {
            // If product not found, try to match similar
            foreach ($catalogoProdutos as $cat => $produtos) {
                foreach ($produtos as $p) {
                    if (stripos($produto, substr($p, 0, 5)) !== false || stripos($p, substr($produto, 0, 5)) !== false) {
                        $categoria = $cat;
                        break 2;
                    }
                }
        ...
        // Add to results
        $dados['linha_categoria'][] = $categoria;
        $dados['linha_produto'][] = $produto;
        $dados['linha_quantidade'][] = $item['quantidade'] ?? 1;
        $dados['linha_num_serie'][] = $item['sn'] ?? '';
    }
